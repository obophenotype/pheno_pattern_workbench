...
class TermGrid extends Grid<OntologyClass> {
    ...
    public TermGrid(OntologyTermSet ts, CandidateKB kb) {
        this.kb = kb;
        this.ts = ts;
        setWidth("100%");
        setHeight("100%");
        setStyleName("termgrid");
        Column<OntologyClass, String> p = addColumn(
                rec -> "<strong>"+rec.getLabel()+"</strong>", new HtmlRenderer()).setCaption("Phenotype");
        
        Column<OntologyClass, Integer> c_compl = addColumn(c->c.indirectChildren().size()).setCaption("Impact").setWidth(70.0);
        
        kb.addGridChangeListener(this::refreshFilter);
        Column<OntologyClass, Button> c_sugg = addComponentColumn(recon -> {
            Button button = new Button("");
            button.addClickListener(click -> suggestSimilar(recon));
            button.setStyleName("termgrid-button");
            return button;
        }).setWidth(25.0).setStyleGenerator(sg).setCaption("S");
        Column<OntologyClass, Button> c_ch = addComponentColumn(recon -> {
            Button button = new Button("");
        ...
        /*p.setExpandRatio(3);
        c_compl.setExpandRatio(2);
        c_sugg.setExpandRatio(1);
        c_ch.setExpandRatio(1);
        c_candidate.setExpandRatio(1);
*/
        //setRowHeight(60);
	...
	private void suggestSimilar(OntologyClass recon) {
        Notification.show(""+ recon.getLabel());
    }
